...
class FormDumperGenerator {
    ...
    protected function getFormOpen($method, $model)
    {
        ...
        {
            return "{{ Form::model(\${$model}, array('method' => 'PATCH', 'class'=>'form-horizontal', 'route' => array('{$models}.update', \${$model}->id))) }}";
        }

        return "{{ Form::open(array('route' => '{$models}.store', 'class'=>'form-horizontal')) }}";
    }
